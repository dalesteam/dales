...
!> Cleaning up after the run
  subroutine exitaerosolmicro
  !*********************************************************************
  ! subroutine exitbulkmicro
  !*********************************************************************
    implicit none

    deallocate(Nc,Nr,Nrp,qltot,qr,qrp,qlm,nuc,thlpmcr,qtpmcr)
  ...
  end subroutine exitaerosolmicro

!> Calculates the microphysical source term.
  subroutine aerosolmicro
...
!      aer_evpc(i,j,k,:) = aer_evpc(i,j,k,:)! Idem /rhof(k)
  ...
  !(l_aertend) then call bulkaertend
  ! end if
       ...
       if ( qcmask(i,j,k) ) then
          ...
          if ( Nc(i,j,k) < ncmin ) then
             ...
             if ( mod_numb(iais_n) + mod_numb(iacs_n) + mod_numb(icos_n) > 0. ) then   
                ...
                dNcdt = max(1.e6/delt*( 0.1*(w0(i,j,k)*100.*Nt/(w0(i,j,k)*100.+.023*Nt))**1.27 - 1.e-6*Nc(i,j,k)), 0. ) !Eq.( ) PN15
                ...
                do imod = 4, 2, -1 !Caution imod is not the proper iterator, but coincidentally iais_n, iacs_n and icos_n = 2,3,4
                   ...
                   elseif ( aer_conc(i,j,k,imod) > 0. ) then
                      ...
                      fm(imod) = 1. - .5*erfc(erfcinv(2.*fn(imod))-3.*log(sigma_lognormal(imod))/sqrt(2.))
                      dNcdt    = 0.0 
                   endif
                enddo ! imod
             end if   ! Nt = 0
  ...
  subroutine cloudevaporation  
    ...
    use modfields, only : ql0, svp, rhof, svm, sv0
