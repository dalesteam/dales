...
!         svplsmn = svplsmn/nsamples
  ...
  !           ----------------
    ...
    if(myid==0)then
      ...
      write (ifoutput,'(A/2A/2A)') &
          ...
          ,'(----(G/KG)------)  (--- (M/S ---)   (-----------)  (---)'
      do k=1,kmax
        write(ifoutput,'(I5,F10.2,F7.1,5F10.4,F12.5,3F11.4,F11.5)') &
            k, &
            zf    (k), &
            presf (k)/100., &
            tmn   (k), &
  ...
  !     -----------------------------------------------------------------------
      ...
      write(ifoutput,'(I5,F10.2,F7.1,6E13.5)') &
            (k, &
            zh       (k), &
            presh     (k)/100., &
            wthlsmn   (k)                              , &
      ...
      write(ifoutput,'(I5,F10.2,F7.1,11E13.5)') &
          (k, &
            zh       (k), &
            presh     (k)/100., &
            uwtmn    (k)                              , &
...
!             qs2mn (k)*1e6, &
  ...
  !----   Write information about scalar field and its tendencies ------
      ...
      do n=1,nsv
        ...
        write (ifoutput,'(2A/A/A/A,I2.2,A,/A/A/2A)') &
            ...
            ,'#----------------------------------------------------------' &
            ,'-------------'
        write(ifoutput,'(I5,2F10.2,E14.5e3,2F10.2,E14.5E3)') &
            (k, &
              zf       (k), &
              presf    (k)/100., &
              svmmn    (k,n), &
  ...
  !        -----------------------
        ...
        write(ifoutput,'(I5,2F10.2,3E14.5E3)') &
            (k, &
              zh       (k), &
              presh    (k)/100., &
              wsvsmn   (k,n)                            , &
