    ...
    if (lnetcdf) then
     ...
     if (myid==0) then
        ...
        do isamp=1,isamptot
          ...
          call ncinfo(ncname(32,:,isamp),'nrtsamphav'//samplname(isamp),trim(longsamplname(isamp))//' '//'','samplinf fraction for gradients','mt')
...
!> Write the statistics to file
  subroutine writesampling
    ...
    allocate( wfmn(k1),thlfmn(k1),thvfmn(k1),qtfmn(k1),qlfmn(k1),nrsampfmn(k1),massflxhmn(k1), &
              wtlthmn(k1),wtvthmn(k1),wqtthmn(k1),wqlthmn(k1),uwthmn(k1),vwthmn(k1),qrfmn(k1))
