...
!> Performs the actual sampling
  subroutine dosampling
    use modglobal, only : zh,i1,i2,j1,j2,kmax,k1,ih,jh,&
...
!
         wadvhavl (k,isamp) = wadvhavl(k,isamp) &
                           - 0.5 * (whav0 (k+1)**2 - whav0 (k-1)**2)/ (zh(k+1) - zh(k-1))  * nrsamph0l(k)
         subphavl (k,isamp) = subphavl (k,isamp) &
                           - (sigh0(k+1) * wwshav0(k+1) - sigh0(k-1)*wwshav0(k-1)) / (zh(k+1) - zh(k-1)) / sigh0 (k)  * nrsamph0l(k)  
       endif
    enddo
...
!normalize variables
        ...
        do k=1,kmax
          if (nrsampf(k,isamp)>0) then
            ...
            wwshmn (k) = wwshav(k,isamp)/nrsampf(k,isamp)
          endif
...
!write files
        ...
        write (ifoutput,'(2A/3A)') &
           ...
           ,'    DTAUDZHMN     DTAUDXHMN    CORIOLIS     RESIDUAL    WS_END       SIG_END      WADVHMN   SUBPLUME      NRTSAMPHAV' &
           ,'  '
        do k=1,kmax
          write(ifoutput,'(i5,F6.0,F7.1,F9.4,13F13.7,i10)') &
              k, &
              zh        (k), &
              presh     (k)/100., &
              nrsamphmn (k),&
