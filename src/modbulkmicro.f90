...
!> Initializes and allocates the arrays
  subroutine initbulkmicro
    ...
    allocate( Nc       (2-ih:i1+ih,2-jh:j1+jh,k1)  &
             ...
             ,qlpmcr   (2-ih:i1+ih,2-jh:j1+jh,k1)  &
    ...
    ! Aerosol+ mode characteristics
    allocate( sigma_lognormal(nmod-2),  & ! Mode width parameter sigma [-]
              spec_k         (nspec-1), & ! Hygroscopicity parameters [-] 
              spec_rho       (nspec-1))   ! Densities [kg m-3]   

    if     ( ipohlker == 1 ) then

      ! PR aerosol population
      sigma_lognormal = (/ 1.584, 1.553, 1.5 /)  
      spec_k          = (/ 0.12,  0.18/)
      spec_rho        = (/ 1800., 1800./)   
     
    elseif ( ipohlker == 2 ) then

      ! LRT aerosol population
      sigma_lognormal = (/ 1.822, 1.682, 1.5 /)  
      spec_k          = (/ 0.18,  0.35 /)
      spec_rho        = (/ 1800., 1800. /)

    elseif ( ipohlker == 3 ) then

      ! BB aerosol population
      sigma_lognormal = (/ 1.221, 1.786, 1.5 /)
      spec_k          = (/ 0.14,  0.17 /)
      spec_rho        = (/ 1800., 1800. /)

    else
      
      STOP "Invalid switch for aerosol properties"
...
!> Cleaning up after the run
  ...
  !*********************************************************************
    ...
    deallocate(Nc,Nr,Nrp,qltot,qr,qrp,nuc,thlpmcr,qtpmcr,qlpmcr)
    ...
    deallocate(sigma_lognormal, spec_k, spec_rho)
...
!> Calculates the microphysical source term.
  subroutine bulkmicro
    ...
    use modfields,        only : sv0,svm,svp,qtp,thlp,ql0,qlp,exnf,rhof,qt0
    ...
    if (l_rain) then
       ...
       do k=1,k1
          ...
          if (qr(i,j,k) > qrmin .and. Nr(i,j,k) > nrmin)  then
             qrmask (i,j,k) = .true.
          else
             qrmask (i,j,k) = .false.
          endif
...
!      aer_evpc(i,j,k,:) = aer_evpc(i,j,k,:)! Idem /rhof(k)
      ...
      do iaer = 1,naer
         ...
         if ( aertest < -eps0 ) then
            if ( imod <= nmod-2 ) then ! Checking and correction for free aerosol sinks
!            write(6,'(A15,3I3,2E11.3)') 'Start balancing', iaer, imod, ispec, aertest, aer_tend(i,j,k,iaer)
            ...
            elseif ( imod == nmod-1 ) then ! Checking and correction for cloud evporation
!               write(6,'(A15,3I3,5E11.3)') 'Start balancing', iaer, imod, ispec, aertest, aer_evpc(i,j,k,iaer), aer_auto(i,j,k,iaer), aer_accr(i,j,k,iaer), aer_slfc(i,j,k,iaer)

               select case ( ispec ) ! Select free aerosol target variable
                  case(1); taer = iacs_n  ! number
                  case(2); taer = iso4acs ! SO4
                  case(3); taer = ipomacs ! POM
               end select
            ...
            elseif ( imod == nmod ) then ! Checking and correction for rain
               ...
               if (aer_evpr(i,j,k,iaer) < -eps0) then
                  ...
                  aer_tend(i,j,k,taer  ) = aer_tend(i,j,k,taer) + fix
                  aer_evpr(i,j,k,taer  ) = aer_evpr(i,j,k,taer) + fix
      ...
      if ((qrtest < qrmin) .or. (nrtest < 0.) ) then ! correction, after Jerome's implementation in Gales
         ...
         qlp(i,j,k)         = qlp(i,j,k)     +  qlpmcr(i,j,k)
      end if
  ...
  subroutine cloudactivation
    ...
    real, dimension(nmod-2) :: mod_numb, mod_mass, mod_vol, mod_rho, mod_k, fn, fm
    ...
    ! Values used to calculate interactive supersaturation    
    !MdB #TEMP >> move to namoptions? 
    logical, parameter :: l_super = .true.

    do j=2,j1
    do i=2,i1
    do k=1,k1    

       if ( qcmask(i,j,k) .or. l_super ) then
          ...
          if (l_kohler) then
             if ( (l_super .and. S0(i,j,k) > 0.) .or. (.not. l_super .and. Nc(i,j,k) > ncmin )) then 
             ...
             !Aggregate free aerosol modes
                 do iaer = 1, naer !Aggregate modes
                     ...
                     if (imod > nmod-2 ) cycle  ! Only free aerosol modes
...
!                         endif
                 ...
                 do imod=1,nmod-2 ! Activation per mode, only free aerosol modes
                     if (mod_mass(imod) < eps0 .or. mod_numb(imod) < eps0 .or. mod_k(imod) == 0.) cycle
                       ...
                       mod_rho(imod) = mod_mass(imod)/mod_vol(imod)                                 
                       mod_k  (imod) = mod_k   (imod)/mod_vol(imod)                                 
!                     mod_rho(imod) = mod_mass(imod)/mod_rho(imod)
!                     mod_k (imod)  = mod_mass(imod)/mod_k (imod)
 
                     d_crit = (4.*A**3./(27.*mod_k(imod)*log(S0(i,j,k)+1.)**2.))**(1./3.) 
...
!             end do
!
!             if ( mod_numb(iais_n) + mod_numb(iacs_n) > 0. ) then   
!!default             if ( mod_numb(iais_n) + mod_numb(iacs_n) + mod_numb(icos_n) > 0. ) then   
...
!           
!                Nt = 1e-6*(f_ais*mod_numb(iais_n) + mod_numb(iacs_n)) !Eq.(4) PN15
!default                Nt = 1e-6*(f_ais*mod_numb(iais_n) + mod_numb(iacs_n) + mod_numb(icos_n)) !Eq.(4) PN15
...
!             end if   ! Nt = 0
          ...
          do iaer= 1,naer
             ...
             if (imod > nmod-2) cycle      ! Exclude cloud and rain modes

             select case ( ispec )    ! Select cloud target variable
                case(1); taer = inc;     fac = fn(imod) ! Number
                case(2); taer = iso4cld; fac = fm(imod) ! SO4
                case(3); taer = ipomcld; fac = fm(imod) ! POM
             end select

!default             select case ( ispec )    ! Select cloud target variable
!                case(1); taer = inc;     fac = fn(imod) ! Number
!                case(2); taer = iso4cld; fac = fm(imod) ! SO4
...
!             end select
  ...
  subroutine cloudevaporation  
    use modglobal, only : i1, j1, k1, rdt
    use modfields, only : ql0, qt0, svp, rhof, qlm, svm, sv0, S0
    ...
    if ( Nc(i,j,k) > ncmin ) then
       ...
       if ( qcmask(i,j,k) ) then
         if ( S0(i,j,k) < -eps0 ) then
           dq = (ql0(i,j,k)-qt0(i,j,k))*S0(i,j,k)/(S0(i,j,k)+1.)     
           frac = max(0.,min(1.,dq/ql0(i,j,k))/rdt)
         else
           frac = 0.
         endif
...
!        endif !qlm>ql0
       ...
       else ! complete evaporation because qcmask = .false.
         frac = 1.
       endif !qcmask
           ...
           do iaer = 1,naer
              ...
              if (imod == nmod-1) then
                select case ( ispec )
                  ...
                  case(3); taer = ipomacs; evap = evapm(ispec-1); Fx = Fm ! POM
                end select 

                aer_evpc(i,j,k,iaer)   = -evap                      
                aer_evpc(i,j,k,taer)   =  evap
                
                aer_tend(i,j,k,iaer)   = aer_tend(i,j,k,iaer)   - evap 
                aer_tend(i,j,k,taer)   = aer_tend(i,j,k,taer)   + evap
              endif                
          enddo !iaer
  ...
  do k=1,k1
     ...
     do iaer = 1,naer
        ...
        ispec = nspec_type(iaer)
         ...
         if (imod > nmod-2 ) cycle !Exclude cloud & rain modes
     ...
     do imod= 1,nmod-2 !Only free aerosol modes
        ...
        if ( (mod_mass(imod) > eps0) .and. (mod_numb(imod) > ncmin)) then
           ...
           if ( qcmask(i,j,k) .and. ( Nc(i,j,k) > ncmin )) then
              ...
              meanaer_inc = max(min(100.*meanaer,8.0E-03),1.5E-8) ! in cm

              scavinc_n(imod) = 1e-6*Nc(i,j,k)*lookup_interpolate(ncld,logcldrad,naer_inc,logaerrad,incnumb,log(meancld),log(meanaer_inc)) 
              scavinc_m(imod) = 1e-6*Nc(i,j,k)*lookup_interpolate(ncld,logcldrad,naer_inc,logaerrad,incmass,log(meancld),log(meanaer_inc))
           ...
           if (l_rain) then
              if ( qrmask(i,j,k) .and. (sed_qr(i,j,k)*3600. > .01) ) then   
                 ...
                 meanaer  = max(min(.9999e3,meanaer*1e6 ),1.5e-3) !in micron 
                 
                 scavblc_n(imod) = lookup_interpolate(nrai,lograinrate,naer_blc,logaerrad_blc,blcnumb,log(rainrate),log(meanaer))
                 scavblc_m(imod) = lookup_interpolate(nrai,lograinrate,naer_blc,logaerrad_blc,blcmass,log(rainrate),log(meanaer))
     ...
     do iaer= 1,naer
        ...
        select case ( ispec )    ! Select cloud/rain target variable
           ...
           case(3); scavinc = scavinc_m(imod); scavblc = scavblc_m(imod); taerinc = ipomcld; taerblc = ipomrai ! POM
        end select

!default        select case ( ispec )    ! Select cloud/rain target variable
!           case(1); scavinc = scavinc_n(imod); scavblc = scavblc_n(imod); taerinc = 0;       taerblc = 0       ! Number
!           case(2); scavinc = scavinc_m(imod); scavblc = scavblc_m(imod); taerinc = iso4cld; taerblc = iso4rai ! SO4
...
!        end select
  ...
  subroutine autoconversion
    ...
    use modmpi,    only : myid
    use modfields, only : exnf,rhof,ql0,qlm,qt0,sv0
    ...
    !
      ...
      do k=1,k1
         if ( qcmask(i,j,k) .and. ( Nc(i,j,k) > ncmin ) ) then
            ...
            xc     (i,j,k) = min(xcmax,rhof(k) * ql0(i,j,k) / Nc(i,j,k)) ! No eps0 necessary
            ...
            !Water & associated heat
            qrp    (i,j,k) = qrp    (i,j,k) + au(i,j,k)
            qlpmcr (i,j,k) = qlpmcr (i,j,k) - au(i,j,k)
            qtpmcr (i,j,k) = qtpmcr (i,j,k) - au(i,j,k)
            thlpmcr(i,j,k) = thlpmcr(i,j,k) + au(i,j,k)*(rlv/(cp*exnf(k)))

            !Particle number
            aer_auto(i,j,k,inc) = - min(au(i,j,k)/xc(i,j,k),Nc(i,j,k)) 
            aer_auto(i,j,k,inr) =   min(au(i,j,k)/x_s,Nc(i,j,k))        
            ...
            !Aerosol mass
            do iaer= 1,naer
               if (nmod_type(iaer) == nmod-1 .and. nspec_type(iaer) > 1) then

                  aer_auto(i,j,k,iaer)   = - au(i,j,k)/ql0(i,j,k)*aer_conc(i,j,k,iaer)
                  aer_auto(i,j,k,iaer+1) = + au(i,j,k)/ql0(i,j,k)*aer_conc(i,j,k,iaer)
      
                  aer_tend(i,j,k,iaer)   = aer_tend(i,j,k,iaer)   + aer_auto(i,j,k,iaer)    
                  aer_tend(i,j,k,iaer+1) = aer_tend(i,j,k,iaer+1) + aer_auto(i,j,k,iaer+1) 
  ...
  !*********************************************************************
    ...
    use modfields, only : exnf,rhof,ql0,qlm,sv0,qt0,qtm
    use modmpi,    only : myid
    implicit none
    ...
    !
     ...
     do k=1,k1
        if (qrmask(i,j,k) .and. qcmask(i,j,k) .and. Nc(i,j,k) > ncmin) then
           ...
           !Cannot take more cloud water than available
           ac(i,j,k) =  min( ql0(i,j,k)/delt , ac(i,j,k)) 

           !Water & associated heat
           qlpmcr (i,j,k) = qlpmcr (i,j,k) - ac(i,j,k)
           qtpmcr (i,j,k) = qtpmcr (i,j,k) - ac(i,j,k)
           qrp    (i,j,k) = qrp    (i,j,k) + ac(i,j,k)
           thlpmcr(i,j,k) = thlpmcr(i,j,k) + ac(i,j,k)*(rlv/(cp*exnf(k)))

           !Number
           aer_accr(i,j,k,inc) = - ac(i,j,k)/xc(i,j,k)     
...
!! flux is calc. anal.
  ...
  subroutine sedimentation_cloud
    ...
    do i=2,i1
      ...
      qlpmcr(i,j,k)  = qlpmcr(i,j,k)  + (sedc(i,j,k+1)-sedc(i,j,k))/(dzf(k)*rhof(k))
      qtpmcr(i,j,k)  = qtpmcr(i,j,k)  + (sedc(i,j,k+1)-sedc(i,j,k))/(dzf(k)*rhof(k))
      thlpmcr(i,j,k) = thlpmcr(i,j,k) - (sedc(i,j,k+1)-sedc(i,j,k))/(dzf(k)*rhof(k))*(rlv/(cp*exnf(k)))
...
!!   polynomial function
  subroutine sedimentation_rain
    ...
    use modfields, only : rhof, svm, sv0, qt0, ql0
    use modmpi,    only : myid
    implicit none
    ...
    do iaer = 1,naer    
       if (nmod_type(iaer) == nmod .and. nspec_type(iaer) > 1) then
...
!
  ...
  !*********************************************************************
    ...
    use modfields, only : exnf,qt0,svm,qvsl,tmp0,ql0,esl,qvsl,rhof, svm, svp, S0

    implicit none
    integer :: i, j, k, iaer, ispec, imod, taer 
    ...
    do k=1,k1
      if (qrmask(i,j,k)) then
        if (l_super) then
           S(i,j,k) = min(0.,S0(i,j,k))
        else
           S(i,j,k) = min(0.,(qt0(i,j,k)-ql0(i,j,k))/qvsl(i,j,k)- 1.)                                     !Heus Eq. (76)
        endif
        G(i,j,k) = (Rv * tmp0(i,j,k)) / (Dv*esl(i,j,k)) + rlv/(Kt*tmp0(i,j,k))*(rlv/(Rv*tmp0(i,j,k)) -1.) !Heus Eq. (77)
        G(i,j,k) = 1./G(i,j,k)                                                                            !Heus Eq. (77)
      endif
    enddo
    enddo
    enddo
    ...
    if ( l_sb ) then
       ...
       do k=1,k1
         if (qrmask(i,j,k)) then
           ...
           F(i,j,k) = avf * mygamma21(numel)*Dvr(i,j,k) +  &                                              ! Heus (2010), Eq. (76)
              bvf*Sc_num**(1./3.)*(a_tvsb/nu_a)**0.5*mygamma251(numel)*Dvr(i,j,k)**(3./2.) * &            ! Heus (2010), Eq. (76)
              (1.-(1./2.)  *(b_tvsb/a_tvsb)    *(lbdr(i,j,k)/(   c_tvsb+lbdr(i,j,k)))**(mur(i,j,k)+2.5) & ! Seifert (2008), Eq. (A7)
                 -(1./8.)  *(b_tvsb/a_tvsb)**2.*(lbdr(i,j,k)/(2.*c_tvsb+lbdr(i,j,k)))**(mur(i,j,k)+2.5) & ! Seifert (2008), Eq. (A7)
                 -(1./16.) *(b_tvsb/a_tvsb)**3.*(lbdr(i,j,k)/(3.*c_tvsb+lbdr(i,j,k)))**(mur(i,j,k)+2.5) & ! Seifert (2008), Eq. (A7)
                 -(5./128.)*(b_tvsb/a_tvsb)**4.*(lbdr(i,j,k)/(4.*c_tvsb+lbdr(i,j,k)))**(mur(i,j,k)+2.5) ) ! Seifert (2008), Eq. (A7)
! *lbdr(i,j,k)**(mur(i,j,k)+1.)/f_gamma_1(i,j,k) factor moved to F
            evap(i,j,k) = 2*pi*Nr(i,j,k)*G(i,j,k)*F(i,j,k)*S(i,j,k)/rhof(k)
            Nevap(i,j,k) = c_Nevap*evap(i,j,k)*rhof(k)/xr(i,j,k) ! Heus (2010), Eq. (78)
         endif
       enddo
       enddo
       enddo
          ...
          if (imod == nmod) then
             ...
             if ( qrmask(i,j,k) ) then
                if ( ispec == 1 ) then
                   ...
                   ! Resuspended aerosol fraction =/= evaporated rain water
                   ...
                   frac = max(0.,min(-evap(i,j,k)/qr(i,j,k)*delt,1.)) !Between 0-1
                   corr = (1. - exp(-2*sqrt(frac))*(1. + 2.*sqrt(frac) + 2.*frac + 4./3.*frac**(3./2.)) )*(1.-frac) + frac**2.
             ...
             ! Limit to available in-rain aerosol   
             ...
             aer_evpr(i,j,k,taer) = evapt
             aer_tend(i,j,k,taer) = aer_tend(i,j,k,taer) + evapt

          endif !imod == nmod, i.e. rain   
       enddo !iaer
