...
!      end do
      ...
      elseif(TRIM(traceGasNameOrder(m))=='CH4')   then
        !cstep ch4(:) = REAL(ch4factor,KIND=kind_rb)*tmpTrace(:)
         ch4(:) = ch4factor
      elseif(TRIM(traceGasNameOrder(m))=='N2O')   then
        !cstep n2o(:) = REAL(n2ofactor,KIND=kind_rb)*tmpTrace(:)
         n2o(:) = n2ofactor
...
!        h2ovmr  (i, krad1)   = mwdry/mwh2o * qv_slice(i,kradmax)
      ...
      do i=1,imax
        do k=1,kradmax
          ...
          qci_slice(i,k) = qci_slice(i,k)*rho_slice(i,k)   !cstep, qci is now in kg/m3
        enddo
...
!  cstep              write (6,*) 'reff = ',i,k,qcl_slice(i,k),liquidRe(i,k)
          ...
          if (IWP_slice(i,k).gt.0) then
             ...
             iceRe (i,k) = 377.4 + 203.3 * B_function + 37.91 * B_function**2 + 2.3696 * B_function**3 !micrometer
          endif
          ...
          !cstep /net/labdata/stephan/Knmi/racmo/SCM_versions/31r1/source/scmall/ifs/phys_ec/radlsw.F90, line 514
          !cstep parameterization gives negative values, this minimum value prevents this
          !cstep if ( (IWP_slice(i,k).gt.0.) .AND. (iceRe(i,k).lt.45)) then !cstep 
          !cstep  iceRe(i,k) = 45.
          !cstep endif
