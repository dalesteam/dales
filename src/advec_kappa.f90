...
subroutine  halflev_kappa(putin,putout)
  ...
  do k=1,k1
    do j=2-jh,j1+jh
      do i=2-ih,i1+ih
      rhoput(i,j,k)=rhobf(k)*putin(i,j,k)
      end do
    end do
  end do
    ...
    do k=3,k1
      do j=2,j1
        do i=2,i1
          if (w0(i,j,k)>=0) then
            d1 = rhoput(i,j,k-1)-rhoput(i,j,k-2)
            d2 = rhoput(i,j,k  )-rhoput(i,j,k-1)
            cf = rhoput(i,j,k-1)
          else
            d1 = rhoput(i,j,k  )-rhoput(i,j,k+1)
            d2 = rhoput(i,j,k-1)-rhoput(i,j,k  )
            cf = rhoput(i,j,k  )
          end if
    ...
    do j=2,j1
      do i=2,i1
        if (w0(i,j,2)>=0) then
          d1 = 0
          d2 = rhoput(i,j,2)-rhoput(i,j,1)
          cf = rhoput(i,j,1)
        else
          d1 = rhoput(i,j,2)-rhoput(i,j,3)
          d2 = rhoput(i,j,1)-rhoput(i,j,2)
          cf = rhoput(i,j,2)
        end if
