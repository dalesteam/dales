...
!> Write the statistics to file
  subroutine writeradstat
      ...
      use modraddata, only : iradiation,irad_par,irad_tenstr,irad_rrtmg
      implicit none
  ...
  !           ----------------
    ...
    if(myid==0)then
     ...
     if((iradiation == irad_par .or. iradiation ==irad_rrtmg) .or. iradiation == irad_tenstr) then ! delta eddington or RRTMG)
