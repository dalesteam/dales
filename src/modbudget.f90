   ...
   !Preparing output files
    ...
    if (lnetcdf) then
      ...
      if (myid==0) then
         ...
         !
         ! unit of TKE when weighted by rho: kg/m^3 * m^2/s^2 = kg/m/s^2 = J/m^3
         !
        call ncinfo(ncname( 1,:),'tker','Resolved TKE','kg/m/s^2','tt')
        call ncinfo(ncname( 2,:),'shr','Resolved Shear','kg/m/s^3','tt')
        call ncinfo(ncname( 3,:),'buo','Resolved Buoyancy','kg/m/s^3','tt')
        call ncinfo(ncname( 4,:),'trsp','Resolved Transport','kg/m/s^3','tt')
        call ncinfo(ncname( 5,:),'ptrsp','Resolved Pressure transport (redistribution)','kg/m/s^3','tt')
        call ncinfo(ncname( 6,:),'diss','Resolved Dissipation','kg/m/s^3','tt')
        call ncinfo(ncname( 7,:),'budg','Resolved Storage = dE/dt','kg/m/s^3','tt')
        call ncinfo(ncname( 8,:),'stor','Resolved Budget = sum of contributions excl storage','kg/m/s^3','tt')
        call ncinfo(ncname( 9,:),'resid','Resolved Residual = budget - storage','kg/m/s^3','tt')
        call ncinfo(ncname(10,:),'sbtke','Subgrid TKE','kg/m/s^2','tt')
        call ncinfo(ncname(11,:),'sbshr','Subgrid Shear','kg/m^2/s^2','tt')
        call ncinfo(ncname(12,:),'sbbuo','Subgrid Buoyancy','kg/m^2/s^2','tt')
        call ncinfo(ncname(13,:),'sbdiss','Subgrid Dissipation','kg/m^2/s^2','tt')
        call ncinfo(ncname(14,:),'sbstor','Subgrid Storage','kg/m^2/s^2','tt')
        call ncinfo(ncname(15,:),'sbbudg','Subgrid Budget = sum of contributions excl storage','kg/m^2/s^2','tt')
        call ncinfo(ncname(16,:),'sbresid','Subgrid Residual = budget - storage','kg/m^2/s^2','tt')
