...
!> Performs the statistics, keeps track of what the tendencies were last time, and what they are this time.
  subroutine samptend(tendterm,firstterm,lastterm)
    ...
    if (lchecktend) call checktend(tendterm)
...
!> Cleans up after the run
  ...
  subroutine printpoint(i,j,k,stage)
    ...
    write (*,*) 'zlt',  zlt(i, j, k)
    write (*,*) 'ekm, ekh',  ekm(i, j, k), ekh(i, j, k)
    write (*,*) 'u0, v0, w0', u0(i, j, k), v0(i, j, k), w0(i, j, k)
    write (*,*) 'rk3step, rdt, delt', rk3step, rdt, delt
    STOP
