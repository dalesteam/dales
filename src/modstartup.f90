...
!----------------------------------------------------------------
  ...
  subroutine init_timedep_from_netcdf(filename, height, time, wqsurft,wtsurft,thlst,qtst,pst,Qnetavt,ugt,vgt,dpdxlt,dpdylt,wflst,dqtdxlst,dqtdylst,dqtdtlst,dthldtlst,dudtlst,dvdtlst,thlpcart,ntimedep,kmax)
    ...
    real(field_r),  intent(out) :: wtsurft(:) !< Surface heat flux.
    real(field_r),  intent(out) :: wqsurft(:) !< Surface moisture flux.
    real(field_r),  intent(out) :: thlst(:) !< Surface liquid water potential temperature.
    real(field_r),  intent(out) :: qtst(:) !< Surface total water mixing ratio.
    real(field_r),  intent(out) :: pst(:) !< Surface pressure.
    real(field_r),  intent(out) :: Qnetavt(:) !< Net surface radiative flux.
    real(field_r),  intent(out) :: ugt(:,:) !< Geostrophic eastward wind.
    real(field_r),  intent(out) :: vgt(:,:) !< Geostrophic northward wind.
    real(field_r),  intent(out) :: dpdxlt(:,:) !< Eastward pressure gradient due to large-scale forcing.
    real(field_r),  intent(out) :: dpdylt(:,:) !< Northward pressure gradient due to large-scale forcing.
    real(field_r),  intent(out) :: wflst(:,:) !< Large-scale subsidence.
    real(field_r),  intent(out) :: dqtdxlst(:,:) !< Eastward gradient of the total water mixing ratio due to large-scale forcing.
    real(field_r),  intent(out) :: dqtdylst(:,:) !< Northward gradient of the total water mixing ratio due to large-scale forcing.
    real(field_r),  intent(out) :: dqtdtlst(:,:) !< Tendency of the total water mixing ratio due to large-scale forcing.
    real(field_r),  intent(out) :: dthldtlst(:,:) !< Tendency of the liquid water potential temperature due to large-scale forcing.
    real(field_r),  intent(out) :: dudtlst(:,:) !< Tendency of the eastward velocity due to large-scale forcing.
    real(field_r),  intent(out) :: dvdtlst(:,:) !< Tendency of the northward velocity due to large-scale forcing.
    real(field_r),  intent(out) :: thlpcart(:,:) !< Tendency of the liquid water potential temperature due to radiative forcing.
