       ...
       do i=2,i1
        !CvH do 2nd order for bottom and top
            ...
            p(i,j,k)  = p(i,j,k)- (  &
                ...
                +(1./rhobf(k))*( &
                  w0(i,j,k+1) * (rhobf(k+1)*a(i,j,k+1)+rhobf(k)*a(i,j,k)) &
                  -w0(i,j,k)  * (rhobf(k-1)*a(i,j,k-1)+rhobf(k)*a(i,j,k)) &
                  ) / ( 2. * dzf(k) ) &
                  )
         end do
      end do
      ...
      do j=2,j1
         do i=2,i1
            ...
            p(i,j,k)  = p(i,j,k)- (  &
                ...
                +(1/rhobf(k))*( &
                      w0(i,j,k+1)/60.&
                      *(37.*(rhobf(k+1)*a(i,j,k+1)+rhobf(k)*a(i,j,k))-8.*(rhobf(k+2)*a(i,j,k+2)+rhobf(k-1)*a(i,j,k-1))&
                          +(rhobf(k+3)*a(i,j,k+3)+rhobf(k-2)*a(i,j,k-2)))&
                      -sign(1._field_r,w0(i,j,k+1))*w0(i,j,k+1)/60.&
                      *(10.*(rhobf(k+1)*a(i,j,k+1)-rhobf(k)*a(i,j,k))-5.*(rhobf(k+2)*a(i,j,k+2)-rhobf(k-1)*a(i,j,k-1))&
                          +(rhobf(k+3)*a(i,j,k+3)-rhobf(k-2)*a(i,j,k-2)))&
                      -w0(i,j,k)  * (rhobf(k-1)*a(i,j,k-1)+rhobf(k)*a(i,j,k))/2. &
                  ) / ( dzf(k) ) &
                  )
         end do
      end do
      ...
      do j=2,j1
         do i=2,i1
            p(i,j,k)  = p(i,j,k)- (  &
                ...
                +(1./rhobf(k))*( &
                      w0(i,j,k+1) * (rhobf(k+1)*a(i,j,k+1)+rhobf(k)*a(i,j,k))/2. &
                      -w0(i,j,k)/60.&
                      *(37.*(rhobf(k)*a(i,j,k)+rhobf(k-1)*a(i,j,k-1))-8.*(rhobf(k+1)*a(i,j,k+1)+rhobf(k-2)*a(i,j,k-2))&
                          +(rhobf(k+2)*a(i,j,k+2)+rhobf(k-3)*a(i,j,k-3)))&
                      +sign(1._field_r,w0(i,j,k))*w0(i,j,k)/60.&
                      *(10.*(rhobf(k)*a(i,j,k)-rhobf(k-1)*a(i,j,k-1))-5.*(rhobf(k+1)*a(i,j,k+1)-rhobf(k-2)*a(i,j,k-2))&
                          +(rhobf(k+2)*a(i,j,k+2)-rhobf(k-3)*a(i,j,k-3)))&
                  ) / dzf(k) &
                  )
         end do
     ...
     if(k==1) then
        do j=2,j1
          ...
          p(i,j,k)  = p(i,j,k)- ( &
              ...
              +(1./rhobf(1))*( &
                ( rhobf(k+1)*a(i,j,k+1) + rhobf(k)*a(i,j,k)) *(w0(i,j,k+1)+ w0(i-1,j,k+1)) &
                ) / (4.*dzf(k)) &
                )
       end do
    end do
    ...
    do j=2,j1
       do i=2,i1
          ...
          p(i,j,k)  = p(i,j,k)-( &
              ...
              + (1./rhobf(k))*( &
                (rhobf(k)*a(i,j,k)+rhobf(k+1)*a(i,j,k+1) )*(w0(i,j,k+1)+w0(i-1,j,k+1)) &
              -(rhobf(k)*a(i,j,k)+rhobf(k-1)*a(i,j,k-1) )*(w0(i,j,k  )+w0(i-1,j,k  )) &
                ) / (4. * dzf(k)) &
                )
       end do
    end do
       ...
       do i=2,i1
          ...
          p(i,j,k)  = p(i,j,k)- ( &
              ...
              +(1/rhobf(k))*( &
                      ...
                      *(10.*(rhobf(k+1)*a(i,j,k+1)-rhobf(k)*a(i,j,k))-5.*(rhobf(k+2)*a(i,j,k+2)-rhobf(k-1)*a(i,j,k-1))&
                          +(rhobf(k+3)*a(i,j,k+3)-rhobf(k-2)*a(i,j,k-2)))&
                      -(rhobf(k)*a(i,j,k)+rhobf(k-1)*a(i,j,k-1) )*(w0(i,j,k  )+w0(i-1,j,k  ))/2. &
                ) / (2. * dzf(k)) &
                )
       end do
    end do
    ...
    do j=2,j1
       do i=2,i1
          ...
          p(i,j,k)  = p(i,j,k)- ( &
                ...
                + (1./rhobf(k))*(&
                      (rhobf(k)*a(i,j,k)+rhobf(k+1)*a(i,j,k+1) )*(w0(i,j,k+1)+w0(i-1,j,k+1))/2. &
     ...
     if(k==1) then
        do j=2,j1
          ...
          p(i,j,k)  = p(i,j,k)- ( &
                ...
                +(1./rhobf(1))*( &
                  (w0(i,j,k+1)+w0(i,j-1,k+1)) *(rhobf(k+1)*a(i,j,k+1)+rhobf(k)*a(i,j,k)) &
                  ) / (4. * dzf(k)) &
                  )
       end do
    end do
    ...
    do j=2,j1
       do i=2,i1
          ...
          p(i,j,k)  = p(i,j,k)- ( &
                ...
                +(1./rhobf(k))*( &
                  (w0(i,j,k+1)+w0(i,j-1,k+1))*(rhobf(k+1)*a(i,j,k+1)+rhobf(k)*a(i,j,k)) &
                -(w0(i,j,k)  +w0(i,j-1,k))  *(rhobf(k-1)*a(i,j,k-1)+rhobf(k)*a(i,j,k)) &
                  ) / (4. * dzf(k)) &
                  )
       end do
    end do
    ...
    do j=2,j1
       do i=2,i1
           ...
           p(i,j,k)  = p(i,j,k)- ( &
                ...
                +(1./rhobf(k))*(&
                      (w0(i,j,k+1)+w0(i,j-1,k+1))*(rhobf(k+1)*a(i,j,k+1)+rhobf(k)*a(i,j,k))/2. &
        ...
        do i=2,i1
          ...
          p(i,j,k)  = p(i,j,k)- ( &
                ...
                +(1/rhobf(k))*( &
                      ...
                      *(10.*(rhobf(k+1)*a(i,j,k+1)-rhobf(k)*a(i,j,k))-5.*(rhobf(k+2)*a(i,j,k+2)-rhobf(k-1)*a(i,j,k-1))&
                          +(rhobf(k+3)*a(i,j,k+3)-rhobf(k-2)*a(i,j,k-2)))&
                      -(w0(i,j,k)  +w0(i,j-1,k))  *(rhobf(k-1)*a(i,j,k-1)+rhobf(k)*a(i,j,k))/2. &
                  ) / (2. * dzf(k)) &
                  )
       end do
    end do
        ...
        do j=2,j1
           do i=2,i1
            ...
            p(i,j,k)  = p(i,j,k)- ( &
                ...
                + (1./rhobh(k))*( &
                  (rhobh(k)*a(i,j,k)+rhobh(k+1)*a(i,j,k+1) )*(w0(i,j,k) + w0(i,j,k+1)) &
                -(rhobh(k)*a(i,j,k)+rhobh(k-1)*a(i,j,k-1) )*(w0(i,j,k) + w0(i,j,k-1)) &
                  )/ (4. * dzh(k)) &
                  )
         end do
         ...
         do i=2,i1
            ...
            p(i,j,k)  = p(i,j,k)- ( &
                ...
                + (1/rhobh(k))*( &
                      ...
                      *(37.*(rhobh(k+1)*a(i,j,k+1)+rhobh(k)*a(i,j,k))-8.*(rhobh(k+2)*a(i,j,k+2)+rhobh(k-1)*a(i,j,k-1))&
                          +(rhobh(k+3)*a(i,j,k+3)+rhobh(k-2)*a(i,j,k-2)))&
                      -sign(1._field_r,(w0(i,j,k)+w0(i,j,k+1)))*(w0(i,j,k)+w0(i,j,k+1))/60.&
                      *(10.*(rhobh(k+1)*a(i,j,k+1)-rhobh(k)*a(i,j,k))-5.*(rhobh(k+2)*a(i,j,k+2)-rhobh(k-1)*a(i,j,k-1))&
                          +(rhobh(k+3)*a(i,j,k+3)-rhobh(k-2)*a(i,j,k-2)))&
                      -(rhobh(k)*a(i,j,k)+rhobh(k-1)*a(i,j,k-1) )*(w0(i,j,k) + w0(i,j,k-1))/2. &
                  )/ (2. * dzh(k)) &
                  )
         end do
      end do
         ...
         do i=2,i1
            ...
            p(i,j,k)  = p(i,j,k)- ( &
                ...
                + (1./rhobh(k))*(&
                  (rhobh(k)*a(i,j,k)+rhobh(k+1)*a(i,j,k+1) )*(w0(i,j,k) + w0(i,j,k+1))/2. &
                  -(w0(i,j,k)+w0(i,j,k-1))/60.&
                  *(37.*(rhobh(k)*a(i,j,k)+rhobh(k-1)*a(i,j,k-1))-8.*(rhobh(k+1)*a(i,j,k+1)+rhobh(k-2)*a(i,j,k-2))&
                      +(rhobh(k+2)*a(i,j,k+2)+rhobh(k-3)*a(i,j,k-3)))&
                  +sign(1._field_r,(w0(i,j,k)+w0(i,j,k-1)))*(w0(i,j,k)+w0(i,j,k-1))/60.&
                  *(10.*(rhobh(k)*a(i,j,k)-rhobh(k-1)*a(i,j,k-1))-5.*(rhobh(k+1)*a(i,j,k+1)-rhobh(k-2)*a(i,j,k-2))&
                      +(rhobh(k+2)*a(i,j,k+2)-rhobh(k-3)*a(i,j,k-3)))&
                  ) / (2. * dzh(k)) &
                  )
         end do
