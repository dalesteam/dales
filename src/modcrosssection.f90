  ...
  subroutine initcrosssection
   ...
   implicit none
    ...
    do cross=1,nxy
      ...
      call ncinfo(ncname2(11,:),'distcld','liquid water sampling distance','-','tt0t')
      call ncinfo(ncname2(12,:),'distcore','core sampling distance','-','tt0t')
...
!~     end do
    if (lnetcdf) then
      allocate(vars(1:imax,1:kmax,15))
      ...
      call writestat_nc(ncid1,15,ncname1(1:15,:),vars,nrec1,imax,kmax)
      deallocate(vars)
    end if
...
!> Do the xy crosssections and dump them to file
  subroutine wrthorz
    ...
    use modfields, only : um,vm,wm,thlm,qtm,svm,thl0,qt0,ql0,exnf,thvf,distcld,distcr
...
!~     end do
    if (lnetcdf) then
      allocate(vars(1:jmax,1:kmax,15))
      ...
      call writestat_nc(ncid3,15,ncname3(1:15,:),vars,nrec3,jmax,kmax)
      deallocate(vars)
    end if
