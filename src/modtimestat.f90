...
integer              :: blh_nsamp = 4        , &              ! Number of samples for fancy BL height calculation
                        ...
                        nRec                                  ! Record number
integer,parameter    :: nVarMax=61                            ! Maximum number of variables (arbitrary, increase if necessary)
real                 :: vars(nVarMax)                         ! Holds the variables to be written
character(80)        :: ncName(nVarMax,4)
...
real                 :: zbasemin             , &              ! Minimum cloud base height [m]
                        tsair_av             , &              ! Surface air temperature [K]
                        rtair                , &              ! Surface total humidity mixing ratio [kg/kg]
                        qvint_av             , &              ! Vertically integrated water vapor [kg/m^2]
                        ...
                        swu_toa              , &              ! Upwelling shortwave radiative flux at the top of the atmosphere [W/m^2]
                        swd_toa              , &              ! Downwelling shortwave radiative flux at the top of the atmosphere [W/m^2]
                        lwu_toa              , &              ! Upwelling longwave radiative flux at the top of the atmosphere [W/m^2]
                        lwd_toa              , &              ! Downwelling shortwave radiative flux at the top of the atmosphere [W/m^2]
                        swu_toa_cs           , &              ! Upwelling clear sky shortwave radiative flux at the top of the atmosphere [W/m^2]
                        swd_toa_cs           , &              ! Downwelling clear sky shortwave radiative flux at the top of the atmosphere [W/m^2]
                        lwu_toa_cs           , &              ! Upwelling clear sky longwave radiative flux at the top of the atmosphere [W/m^2]
                        lwd_toa_cs           , &              ! Downwelling clear sky shortwave radiative flux at the top of the atmosphere [W/m^2]
                        swu_sfc              , &              ! Upwelling shortwave radiative flux at the surface [W/m^2]
                        swd_sfc              , &              ! Downwelling shortwave radiative flux at the surface [W/m^2]
                        lwu_sfc              , &              ! Upwelling longwave radiative flux at the surface [W/m^2]
                        lwd_sfc              , &              ! Downwelling shortwave radiative flux at the surface [W/m^2]
                        swu_sfc_cs           , &              ! Upwelling clear sky shortwave radiative flux at the surface [W/m^2]
                        swd_sfc_cs           , &              ! Downwelling clear sky shortwave radiative flux at the surface [W/m^2]
                        lwu_sfc_cs           , &              ! Upwelling clear sky longwave radiative flux at the surface [W/m^2]
                        lwd_sfc_cs           , &              ! Downwelling clear sky shortwave radiative flux at the surface [W/m^2]
...
!====================================================================================================;
  ...
  subroutine calcTimestat(n)
    ...
    use modfields,    only : rhof,ql0,u0,v0,w0,u0av,v0av,e120,sv0,&
                             ustar,thlflux,qtflux,tskin
    use modsurfdata,  only : isurf,wtsurf,wqsurf,qts,thls,z0, &
                             Qnet,H,LE,G0,tendskin,rs,ra,cliq,wl,rssoil,rsveg,oblav
    use modmicrodata, only : imicro,imicro_bulk,iqr
    ...
    do j=2,j1
      do i=2,i1
        ...
        wmaxl  = max(wmaxl ,maxval( w0(i,j,1:kmax)))
        qlmaxl = max(qlmaxl,maxval(ql0(i,j,1:kmax)))
...
!===========================================================================================================!
  ...
  subroutine calcTimestatUser(n)
    use modstat_nc,  only           : ncinfo,lnetcdf
    use modglobal,   only           : kmax,i1,j1,rslabs,dzf,pref0,rd,cp
    use modsurfdata, only           : albedo,ps
    use modraddata,  only           : lwu,lwd,swu,swd,swuca,swdca,lwuca,lwdca,SW_up_TOA,SW_dn_TOA,LW_up_TOA,LW_dn_TOA, &
                                      SW_up_ca_TOA,SW_dn_ca_TOA,LW_up_ca_TOA,LW_dn_ca_TOA
    use modfields,   only           : thl0,ql0,qt0,sv0,rhof,presf
    use modmicrodata,only           : precep,epscloud,epsqr,epsprec,iqr,iNr,Nc_0, &
                                      imicro, imicro_bulk
...
!      intent(inout), optional      :: ncName
    ...
    integer                        :: cld_boxesl         , & 
                                      ...
                                      rain_boxesl        , &
                                      rain_boxes
    real                           :: swu_tod            , &       ! Average shortwave up at domain top (kmax)
                                      swd_tod                      ! Average shortwave down at domain top (kmax)
    integer                        :: i,j,k

    prec_cntl=0.; Nc_avl=0.; Nr_avl=0.; tsair_avl=0.; qvint_avl = 0.
    cld_boxesl=0; rain_boxesl=0;
    ...
    if (lnetcdf .and. .not. isInitTimestatUser) then
      ...
      call ncinfo(ncName(n,:),'ps','Surface pressure','Pa','time');                                             n=n+1
      call ncinfo(ncName(n,:),'tsair','Surface air temperature (temperature at first model level)','K','time'); n=n+1
      call ncinfo(ncName(n,:),'rtair','Surface total mixing ratio','kg/kg','time');                             n=n+1
      call ncinfo(ncName(n,:),'wvp','Water vapor path','kg/m^2','time');                                        n=n+1
      ...
      call ncinfo(ncName(n,:),'swu_toa','ToA upwelling shortwave radiative flux','-','time');                   n=n+1
      call ncinfo(ncName(n,:),'swd_toa','ToA downwelling shortwave radiative flux','-','time');                 n=n+1
      call ncinfo(ncName(n,:),'lwu_toa','ToA upwelling longwave radiative flux','-','time');                    n=n+1
      call ncinfo(ncName(n,:),'lwd_toa','ToA downwelling longwave radiative flux','-','time');                  n=n+1
      call ncinfo(ncName(n,:),'swu_toa_cs','ToA upwelling clear sky shortwave radiative flux','-','time');      n=n+1
      call ncinfo(ncName(n,:),'swd_toa_cs','ToA downwelling clear sky shortwave radiative flux','-','time');    n=n+1
      call ncinfo(ncName(n,:),'lwu_toa_cs','ToA upwelling clear sky longwave radiative flux','-','time');       n=n+1
      call ncinfo(ncName(n,:),'lwd_toa_cs','ToA downwelling clear sky longwave radiative flux','-','time');     n=n+1
      call ncinfo(ncName(n,:),'swu_sfc','Surface upwelling shortwave radiative flux','-','time');               n=n+1
      call ncinfo(ncName(n,:),'swd_sfc','Surface downwelling shortwave radiative flux','-','time');             n=n+1
      call ncinfo(ncName(n,:),'lwu_sfc','Surface upwelling longwave radiative flux','-','time');                n=n+1
      call ncinfo(ncName(n,:),'lwd_sfc','Surface downwelling longwave radiative flux','-','time');              n=n+1
      call ncinfo(ncName(n,:),'swu_sfc_cs','Surface upwelling clear sky shortwave radiative flux','-','time');  n=n+1
      call ncinfo(ncName(n,:),'swd_sfc_cs','Surface downwelling clear sky shortwave radiative flux','-','time');n=n+1
      call ncinfo(ncName(n,:),'lwu_sfc_cs','Surface upwelling clear sky longwave radiative flux','-','time');   n=n+1
      call ncinfo(ncName(n,:),'lwd_sfc_cs','Surface downwelling clear sky longwave radiative flux','-','time'); n=n+1
    ...
    call horAverage( swu(2:i1,2:j1,kmax), swu_tod  )
    call horAverage( swd(2:i1,2:j1,kmax), swd_tod  )
    if (swd_tod < 0.) then
      alb_tod = - swu_tod / swd_tod
    else
      alb_tod = missVal
    end if
    
    ! Albedo at the top of the atmosphere
    call horAverage( SW_up_TOA(2:i1,2:j1), swu_toa  )
    call horAverage( SW_dn_TOA(2:i1,2:j1), swd_toa  )
    call horAverage( LW_up_TOA(2:i1,2:j1), lwu_toa  )
    call horAverage( LW_dn_TOA(2:i1,2:j1), lwd_toa  )

    call horAverage( SW_up_ca_TOA(2:i1,2:j1), swu_toa_cs )
    call horAverage( SW_dn_ca_TOA(2:i1,2:j1), swd_toa_cs )
    call horAverage( LW_up_ca_TOA(2:i1,2:j1), lwu_toa_cs )
    call horAverage( LW_dn_ca_TOA(2:i1,2:j1), lwd_toa_cs )

    if (swd_toa < 0.) then
      alb_toa = - swu_toa / swd_toa
    else
      alb_toa = missVal
    end if
    ...
    call horAverage( swu(2:i1,2:j1,1), swu_sfc  )
    call horAverage( swd(2:i1,2:j1,1), swd_sfc  )
    call horAverage( lwu(2:i1,2:j1,1), lwu_sfc  )
    call horAverage( lwd(2:i1,2:j1,1), lwd_sfc  )

    ! Clear sky radiative fluxes at the surface
    call horAverage( swuca(2:i1,2:j1,1), swu_sfc_cs  )
    call horAverage( swdca(2:i1,2:j1,1), swd_sfc_cs  )
    call horAverage( lwuca(2:i1,2:j1,1), lwu_sfc_cs  )
    call horAverage( lwdca(2:i1,2:j1,1), lwd_sfc_cs  )

    ! Calculate WVP
    do i=2,i1; do j=2,j1; do k=1,kmax
      qvint_avl   = qvint_avl + (qt0(i,j,k)-ql0(i,j,k))*rhof(k)*dzf(k)
    end do; end do; end do
    call MPI_ALLREDUCE(qvint_avl , qvint_av , 1, MY_REAL, MPI_SUM, comm3d,mpierr)
    qvint_av = qvint_av/rslabs

    ! Calculate the surface pressure and the surface air temperature
    exner = (presf(1) / pref0)**(rd/cp)
    do i=2,i1; do j=2,j1
      tsair_avl = tsair_avl + thl0(i,j,1)*exner
    end do; end do
    call MPI_ALLREDUCE(tsair_avl , tsair_av , 1, MY_REAL, MPI_SUM, comm3d,mpierr)
    tsair_av = tsair_av/rslabs

    ! Determine the mixing ratio at the lowest model level
    call horAverage( qt0(2:i1,2:j1,1)/(1+qt0(2:i1,2:j1,1)), rtair )
    ...
    if (imicro == imicro_bulk) then
      ...
      do i=2,i1
        ...
        if ( sv0(i,j,k,iqr) > epsqr ) then
          Nr_avl      = Nr_avl + sv0(i,j,k,iNr)
    ...
    vars(n) = ps;           n=n+1
    vars(n) = tsair_av;     n=n+1
    vars(n) = rtair;        n=n+1
    vars(n) = qvint_av;     n=n+1
    vars(n) = qlintvar;     n=n+1
    vars(n) = alb_sfc;      n=n+1
    vars(n) = alb_tod;      n=n+1
    vars(n) = alb_toa;      n=n+1
    vars(n) = swu_toa;      n=n+1
    vars(n) = swd_toa;      n=n+1
    vars(n) = lwu_toa;      n=n+1
    vars(n) = lwd_toa;      n=n+1
    vars(n) = swu_toa_cs;   n=n+1
    vars(n) = swd_toa_cs;   n=n+1
    vars(n) = lwu_toa_cs;   n=n+1
    vars(n) = lwd_toa_cs;   n=n+1
    vars(n) = swu_sfc;      n=n+1
    vars(n) = swd_sfc;      n=n+1
    vars(n) = lwu_sfc;      n=n+1
    vars(n) = lwd_sfc;      n=n+1
    vars(n) = swu_sfc_cs;   n=n+1
    vars(n) = swd_sfc_cs;   n=n+1
    vars(n) = lwu_sfc_cs;   n=n+1
    vars(n) = lwd_sfc_cs;   n=n+1
