...
! just let one row or column of processors do less points
    ...
    ! how many data elements are in use on the current node?
    ! if the number is not divisible by nprocx or nprocy
    ! some process may have fewer elements, ans some processes may have *no* elements
...
!    end if
    ...
    if (myidx == nprocx-1) qe = jonx_me
    if (myidy == nprocy-1) pe = iony_me
