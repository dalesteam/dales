module modhipfft
  ...
  save
    ...
    integer :: istat !< hipFFT return status 
    ...
    type(c_ptr) :: planx, planxi, plany, planyi !< Plan handles

    integer(c_size_t) :: worksize, max_worksize !< Size of the required workspace

  contains
    !< Setup plans, workspace, etc
    subroutine hipfftinit(p, Fp, d, xyrt, ps, pe, qs, qe)
      use hipfort_hipfft
      ...
      HIPFFT_FWD_TYPE = HIPFFT_R2C
      HIPFFT_BWD_TYPE = HIPFFT_C2R
#else
      HIPFFT_FWD_TYPE = HIPFFT_D2Z
      HIPFFT_BWD_TYPE = HIPFFT_Z2D
#endif
      ...
      pwork_0 => workspace_0
      istat = hipfftSetWorkArea(planx,  c_loc(workspace_0))
      istat = hipfftSetWorkArea(planxi, c_loc(workspace_0))
      istat = hipfftSetWorkArea(plany,  c_loc(workspace_0))
      istat = hipfftSetWorkArea(planyi, c_loc(workspace_0))
      !$acc end host_data
    ...
    end subroutine hipfftinit

    !< Exit routine
    subroutine hipfftexit(p, Fp, d, xyrt)
      use hipfort_hipfft
    ...
    subroutine init_factors(xyrt)
      ...
      ! hipFFT orders the Fourier coefficients like this:
      !   
    ...
    subroutine hipfftf(p, Fp)
      ...
      call timer_tic('modhipfft/hipfftf', 1)
      
      call transpose_a1(p, px)

      !$acc host_data use_device(px)
#if POIS_PRECISION==32
      istat = hipfftExecR2C(planx, c_loc(px), c_loc(px))
#else
      istat = hipfftExecD2Z(planx, c_loc(px), c_loc(px))
#endif
      !$acc end host_data
...
#if POIS_PRECISION==32
      istat = hipfftExecR2C(plany, c_loc(py), c_loc(py))
#else
      istat = hipfftExecD2Z(plany, c_loc(py), c_loc(py))
#endif
      !$acc end host_data
      ...
      call timer_toc('modhipfft/hipfftf')

    end subroutine hipfftf

    !< Backward transforms
    subroutine hipfftb(p, Fp)
      use hipfort_hipfft
      ...
      call timer_tic('modhipfft/hipfftb', 1)
      ...
      !$acc host_data use_device(py)
#if POIS_PRECISION==32
      istat = hipfftExecC2R(planyi, c_loc(py), pc_loc(y))
#else
      istat = hipfftExecZ2D(planyi, c_loc(py),  c_loc(py))
#endif
      !$acc end host_data
      ...
      !$acc host_data use_device(px)
#if POIS_PRECISION==32
      istat = hipfftExecC2R(planxi, c_loc(px), c_loc(px))
#else
      istat = hipfftExecZ2D(planxi, c_loc(px), c_loc(px))
#endif
      !$acc end host_data
      ...
      call timer_toc('modhipfft/hipfftb')

    end subroutine hipfftb
    ...
    !< Checks the exitcode of hipFFT calls
    subroutine check_exitcode(istat)
      implicit none
      integer, intent(in) :: istat
      
      if ( istat /= 0 ) then
        write(*,*) "hipFFT returned nonzero exitcode: ", istat
        stop
      end if
  ...
  contains
    ...
    subroutine error_and_exit
      write(*,*) "DALES was compiled without GPU support, but hipFFT solver was selected"
      write(*,*) "Use another solver (solver_id), or compile with SYST=Cray-OpenACC"
      call exit(-1)
    end subroutine

#endif

end module modhipfft
