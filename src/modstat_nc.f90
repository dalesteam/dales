...
!Soil fields
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ! This function puts a field in an already open NetCDF file        !
  ! JvdD                                                             !
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine ncfielddump(fName,nVar,ncFieldInfo,ktop)
    use mpi
    use modglobal   , only : imax,jmax,jtot,kmax,i1,j1,k1,dx,dy,zf,zh
    ...
    integer, intent(in)                        :: nVar ,&     ! Number of variables to be written
                                                  ktop        ! Highest full level included in fields
    character(80),dimension(nVar,4),intent(in) :: ncFieldInfo ! Contains info about the variables
    ...
    integer            :: ncFormat(nVar),mask_start(3),mask_count(3),map(3)
    integer            :: n,i
   ...
   ! write(string(26:37),'2(I2.2,":"),I2.2,".",I3.3') values(5:8)
    ...
    call nchandle_error( nf90_put_att(id_fielddump,VarID,'longname','West-east displacement') )
    call nchandle_error( nf90_put_att(id_fielddump,VarID,'units','m') )

    call nchandle_error( nf90_def_var(id_fielddump,'xh',NF90_FLOAT,xhID,VarID) )
    call nchandle_error( nf90_put_att(id_fielddump,VarID,'longname','West-east displacement (0,dx,2dx etc)') )
    call nchandle_error( nf90_put_att(id_fielddump,VarID,'units','m') )

    call nchandle_error( nf90_def_var(id_fielddump,'y',NF90_FLOAT,yID,VarID) )
    call nchandle_error( nf90_put_att(id_fielddump,VarID,'longname','South-north displacement') )
    call nchandle_error( nf90_put_att(id_fielddump,VarID,'units','m') )

    call nchandle_error( nf90_def_var(id_fielddump,'yh',NF90_FLOAT,yhID,VarID) )
    call nchandle_error( nf90_put_att(id_fielddump,VarID,'longname','South-north displacement (0,dy,2dy etc)') )
    call nchandle_error( nf90_put_att(id_fielddump,VarID,'units','m') )

    call nchandle_error( nf90_def_var(id_fielddump,'z',NF90_FLOAT,zID,VarID) )
    call nchandle_error( nf90_put_att(id_fielddump,VarID,'longname','Vertical displacement (full levels)') )
    call nchandle_error( nf90_put_att(id_fielddump,VarID,'units','m') )

    call nchandle_error( nf90_def_var(id_fielddump,'zh',NF90_FLOAT,zhID,VarID) )
    call nchandle_error( nf90_put_att(id_fielddump,VarID,'longname','Vertical displacement (half levels)') )
    call nchandle_error( nf90_put_att(id_fielddump,VarID,'units','m') )

    ! Define possible combinations of dimensions.
    dim_xyz   = (/xID ,yID ,zID /)      ! thermofields (qt,ql,thl etc)
    dim_xyzh  = (/xID ,yID ,zhId/)      ! w velocity
    dim_xhyhz = (/xhID,yhID,zID /)      ! u and v velocities
    dim_z     = (/zId/)                 ! vertical profiles (height, pres)
    dim_zh    = (/zhID/)                ! vertical profiles at half levels

    ncFormat(:) = NF90_FLOAT
    ! Define actual variables
    do n=1,nVar
      select case(trim(ncFieldInfo(n,4)))
        case ('xyz')
          call nchandle_error( nf90_def_var(id_fielddump,ncFieldInfo(n,1),ncFormat(n),dim_xyz  ,VarID) )
        case ('xyzh')
          call nchandle_error( nf90_def_var(id_fielddump,ncFieldInfo(n,1),ncFormat(n),dim_xyzh ,VarID) )
        case ('xhyhz')
          call nchandle_error( nf90_def_var(id_fielddump,ncFieldInfo(n,1),ncFormat(n),dim_xhyhz,VarID) )
        case ('z')
          call nchandle_error( nf90_def_var(id_fielddump,ncFieldInfo(n,1),ncFormat(n),dim_z    ,VarID) )
        case ('zh')
          call nchandle_error( nf90_def_var(id_fielddump,ncFieldInfo(n,1),ncFormat(n),dim_zh   ,VarID) )
        case default
          write(*,*) 'Error in variable: ',trim(ncFieldInfo(n,1))
          stop 'ERROR: Bad dimensional information for restart files'
      end select
      call nchandle_error( nf90_put_att(id_fielddump,VarID,'longname',ncFieldInfo(n,2)) )
      call nchandle_error( nf90_put_att(id_fielddump,VarID,'units'   ,ncFieldInfo(n,3)) )
    end do

    ! Stop definition fase
    call nchandle_error( nf90_enddef(id_fielddump) )

    ! Variables can now be put inside the defined variables in the NetCDF file
    ! Starting with the dimensions
    call nchandle_error( nf90_inq_varid(id_fielddump, 'x', varID)                             )
    call nchandle_error( nf90_put_var(id_fielddump, varID, (/(dx*(i-0.5),i=1,imax)/)  ,(/1/)) )
    call nchandle_error( nf90_inq_varid(id_fielddump, 'xh', varID)                            )
    call nchandle_error( nf90_put_var(id_fielddump, varID, (/(dx*(i-1),i=1,i1)/)      ,(/1/)) )
    call nchandle_error( nf90_inq_varid(id_fielddump, 'y', varID)                             )
    call nchandle_error( nf90_put_var(id_fielddump, varID, (/(dy*(i-0.5),i=1,jtot)/)  ,(/1/)) )
    call nchandle_error( nf90_inq_varid(id_fielddump, 'yh', varID)                            )
    call nchandle_error( nf90_put_var(id_fielddump, varID, (/(dy*(i-1),i=1,jtot+1)/)  ,(/1/)) )
    call nchandle_error( nf90_inq_varid(id_fielddump, 'z', varID)                             )
    call nchandle_error( nf90_put_var(id_fielddump, varID, zf(1:ktop)                 ,(/1/)) )
    call nchandle_error( nf90_inq_varid(id_fielddump, 'zh', varID)                            )
    call nchandle_error( nf90_put_var(id_fielddump, varID, zh(1:ktop+1)               ,(/1/)) )

    ! Make masks for the variables
    mask_start = (/1,myid*jmax+1,1/)
    mask_count = (/imax,jmax,ktop/)

    CALL SYSTEM_CLOCK(t1, count_rate, count_max)

    ! Put the variables in the file
    call nchandle_error( nf90_inq_varid(id_fielddump, ncFieldInfo(1,1), varID) )
    call nchandle_error( nf90_put_var  (id_fielddump, varID, zf(1:ktop) ) )
    call nchandle_error( nf90_inq_varid(id_fielddump, ncFieldInfo(2,1), varID) )
    call nchandle_error( nf90_put_var  (id_fielddump, varID, zh(1:ktop+1)))
    call nchandle_error( nf90_inq_varid(id_fielddump, ncFieldInfo(3,1), varID) )
    call nchandle_error( nf90_put_var  (id_fielddump, varID, presf(1:ktop)))
    call nchandle_error( nf90_inq_varid(id_fielddump, ncFieldInfo(4,1), varID) )
    call nchandle_error( nf90_put_var  (id_fielddump, varID, presh(1:ktop)))

    ! Collective access causes all reads/writes to happen together which is MUCH faster!
    call nchandle_error( nf90_inq_varid(id_fielddump, ncFieldInfo(5,1), varID)           )
    call nchandle_error( nf90_var_par_access(id_fielddump, varID, NF90_COLLECTIVE)       )
    call nchandle_error( nf90_put_var  (id_fielddump, varID, u0(1:i1,1:j1,1:ktop),       &
                                          start=mask_start, count=(/i1,j1,ktop/))        )
    if (myid==0) write(*,*) 'finished writing u'
    call nchandle_error( nf90_inq_varid(id_fielddump, ncFieldInfo(6,1), varID)           )
    call nchandle_error( nf90_var_par_access(id_fielddump, varID, NF90_COLLECTIVE)       )
    call nchandle_error( nf90_put_var  (id_fielddump, varID, v0(1:i1,1:j1,1:ktop),       &
                                          start=mask_start, count=(/i1,j1,ktop/))        )
    if (myid==0) write(*,*) 'finished writing v'
    ! Note that with the combination of mask_start and count in u and v, plane j1 for each
    ! processor is written twice except for the last processor.
    call nchandle_error( nf90_inq_varid(id_fielddump, ncFieldInfo(7,1), varID)           )
    call nchandle_error( nf90_var_par_access(id_fielddump, varID, NF90_COLLECTIVE)       )
    call nchandle_error( nf90_put_var  (id_fielddump, varID, w0(2:i1,2:j1,1:ktop+1),     &
                                          start=mask_start, count=(/imax,jmax,ktop+1/))  )
    if (myid==0) write(*,*) 'finished writing w'
    call nchandle_error( nf90_inq_varid(id_fielddump, ncFieldInfo(8,1), varID)           )
    call nchandle_error( nf90_var_par_access(id_fielddump, varID, NF90_COLLECTIVE)       )
    call nchandle_error( nf90_put_var  (id_fielddump, varID, thl0(2:i1,2:j1,1:ktop),     &
                                          start=mask_start, count=mask_count)            )
    if (myid==0) write(*,*) 'finished writing thl'
    call nchandle_error( nf90_inq_varid(id_fielddump, ncFieldInfo(9,1), varID)           )
    call nchandle_error( nf90_var_par_access(id_fielddump, varID, NF90_COLLECTIVE)       )
    call nchandle_error( nf90_put_var  (id_fielddump, varID, qt0(2:i1,2:j1,1:ktop),      &
                                          start=mask_start, count=mask_count)            )
    if (myid==0) write(*,*) 'finished writing qt'
    call nchandle_error( nf90_inq_varid(id_fielddump, ncFieldInfo(10,1), varID)          )
    call nchandle_error( nf90_var_par_access(id_fielddump, varID, NF90_COLLECTIVE)       )
    call nchandle_error( nf90_put_var  (id_fielddump, varID, ql0(2:i1,2:j1,1:ktop),      &
                                          start=mask_start, count=mask_count)            )
    if (myid==0) write(*,*) 'finished writing ql'
    call nchandle_error( nf90_inq_varid(id_fielddump, ncFieldInfo(11,1), varID)          )
    call nchandle_error( nf90_var_par_access(id_fielddump, varID, NF90_COLLECTIVE)       )
    ! NOTE: e120 is the sqrt of the subgrid tke. Therefore, value is squared.
    ...
    if (nVar > 12) then
      ...
      call nchandle_error( nf90_put_var  (id_fielddump, varID, Nr(2:i1,2:j1,1:ktop),       &
                                            start=mask_start, count=mask_count)            )
      if (myid==0) write(*,*) 'finished writing Nr'
    end if
    ...
    call nchandle_error( nf90_close(id_fielddump) )

  end subroutine ncfielddump
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
