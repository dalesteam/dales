...
subroutine advecc_hybrid(pin,pout)
  ...
  pfacez(2:i1,2:j1,2:3)     = ( pin(2:i1,2:j1,2:3)+pin(2:i1,2:j1,1:2) )/2    !note that pin and pfacez do not have the same dimensions!
  pfacez(2:i1,2:j1,kmax:k1) = ( pin(2:i1,2:j1,kmax:k1)+pin(2:i1,2:j1,kmax-1:kmax) )/2
  ...
  do j=2,j1+1
    ...
    do i=2,i1+1
      ...
      do k=4,kmax-1
        ...
        pfacex(i,j,k) = ip_hybrid(pin(im3:ip2,j,k),u0(i,j,k)>=0.,lsmx(i,j,k))
        pfacey(i,j,k) = ip_hybrid(pin(i,jm3:jp2,k),v0(i,j,k)>=0.,lsmy(i,j,k))
        pfacez(i,j,k) = ip_hybrid(pin(i,j,km3:kp2),w0(i,j,k)>=0.,lsmz(i,j,k))
      end do !Loop over k
    end do !Loop over i
  end do !Loop over j
  ...
  do k=1,kmax
    pout(2:i1,2:j1,k) = pout(2:i1,2:j1,k) - ( &
                             ...
                             +(w0(2:i1,2:j1,k+1)*pfacez(2:i1,2:j1,k+1) -    &
