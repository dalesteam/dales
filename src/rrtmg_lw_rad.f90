...
!      real(kind=rb), intent(in) :: asmcld(:,:,:)     ! In-cloud asymmetry parameter
                                                      ...
                                                      !   for future expansion
                                                      !   (lw scattering not yet available)
!      real(kind=rb),allocatable,dimension(:,:,:)  :: tauaer      ! aerosol optical depth (Dummy variable)
...
! Atmosphere
      real(kind=rb) :: emis(imax,nbndlw)      ! Surface emissivity, constant = 0.95
      ...
      real(kind=rb) :: coldry(nzrad+2)         ! dry air column density (mol/cm2)
      real(kind=rb) :: wbrodl(nzrad+2)         ! broadening gas column density (mol/cm2)
      real(kind=rb) :: wkl(mxmol,nzrad+2)      ! molecular amounts (mol/cm-2)
      real(kind=rb) :: wx(maxxsec,nzrad+2)     ! cross-section amounts (mol/cm-2)
      real(kind=rb) :: pwvcm                  ! precipitable water vapor (cm)
      real(kind=rb) :: semiss(nbndlw)         ! lw surface emissivity
      real(kind=rb) :: fracs(nzrad+2,ngptlw)   !
      real(kind=rb) :: taug(nzrad+2,ngptlw)    ! gaseous optical depths
...
! Atmosphere - setcoef
      ...
      integer(kind=im) :: jp(nzrad+2)          ! lookup table index
      integer(kind=im) :: jt(nzrad+2)          ! lookup table index
      integer(kind=im) :: jt1(nzrad+2)         ! lookup table index
      real(kind=rb) :: planklay(nzrad+2,nbndlw)!
      real(kind=rb) :: planklev(0:nzrad+2,nbndlw)!
      ...
      real(kind=rb) :: colco(nzrad+2)          ! column amount (co)
      real(kind=rb) :: colch4(nzrad+2)         ! column amount (ch4)
      real(kind=rb) :: colo2(nzrad+2)          ! column amount (o2)
      real(kind=rb) :: colbrd(nzrad+2)         ! column amount (broadening gases)
      ...
      integer(kind=im) :: indminor(nzrad+2)
      real(kind=rb) :: minorfrac(nzrad+2)
      real(kind=rb) :: scaleminor(nzrad+2)
      real(kind=rb) :: scaleminorn2(nzrad+2)

      real(kind=rb) :: &                      !
                         fac00(nzrad+2), fac01(nzrad+2), &
                         fac10(nzrad+2), fac11(nzrad+2)
      real(kind=rb) :: &                      !
                         rat_h2oco2(nzrad+2),rat_h2oco2_1(nzrad+2), &
                         rat_h2oo3(nzrad+2),rat_h2oo3_1(nzrad+2), &
                         rat_h2on2o(nzrad+2),rat_h2on2o_1(nzrad+2), &
                         rat_h2och4(nzrad+2),rat_h2och4_1(nzrad+2), &
                         rat_n2oco2(nzrad+2),rat_n2oco2_1(nzrad+2), &
                         rat_o3co2(nzrad+2),rat_o3co2_1(nzrad+2)
...
!      real(kind=rb) :: asmc(nbndlw,nzrad+2)   ! in-cloud asymmetry parameter
                                              !   for future expansion
                                              !   (lw scattering not yet available)
      real(kind=rb) :: ciwp(nzrad+2)           ! cloud ice water path
      real(kind=rb) :: clwp(nzrad+2)           ! cloud liquid water path
      ...
      real(kind=rb) :: taucloud(nzrad+2,nbndlw)! layer in-cloud optical depth

! Output
      real(kind=rb) :: totuflux(0:nzrad+2)     ! upward longwave flux (w/m2)
      real(kind=rb) :: totdflux(0:nzrad+2)     ! downward longwave flux (w/m2)
      real(kind=rb) :: fnet(0:nzrad+2)         ! net longwave flux (w/m2)
      real(kind=rb) :: htr(0:nzrad+2)          ! longwave heating rate (k/day)
      real(kind=rb) :: totuclfl(0:nzrad+2)     ! clear sky upward longwave flux (w/m2)
      real(kind=rb) :: totdclfl(0:nzrad+2)     ! clear sky downward longwave flux (w/m2)
      real(kind=rb) :: fnetc(0:nzrad+2)        ! clear sky net longwave flux (w/m2)
      real(kind=rb) :: htrc(0:nzrad+2)         ! clear sky longwave heating rate (k/day)

      real(kind=rb) :: taucld(nbndlw,imax,nzrad+2)
      real(kind=rb) :: tauaer(imax,nzrad+2,nbndlw)
...
! Set default ioverlap to select of clear or cloud calculation and cloud overlap method
! ioverlap = 0, clear only
! ioverlap = 1, with clouds using random cloud overlap
! ioverlap = 2, with clouds using maximum/random cloud overlap
...
!  other input parameters.
         ...
         call inatm (iplon, nzrad+1, ioverlap, iaer, &
...
!              if(myid==0) write(*,*) 'inside layer loop, k=',k
              ...
              lwUp_slice(iplon,k+1)     = totuflux(k)
              lwDown_slice(iplon,k+1)   = totdflux(k)
              lwUpCS_slice(iplon,k+1)   = totuclfl(k)
