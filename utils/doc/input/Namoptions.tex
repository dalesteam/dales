...
%\usepackage{array}
\usepackage{xcolor}             %
\hypersetup{                    % color text of links instead of surrounding them with red or green boxes
    colorlinks,
...
\end{tabular}} & \multirow{11}{*}{\begin{tabular}{@{\hspace{0cm}}p{4cm}}$x \in \{1,$ $2,$ $5,$ $52,$ $55,$ $6,$ $62,$ $7\}$\end{tabular}}	& Advection scheme for momentum, TKE, $\theta_l$, $q_t$ and scalars: & \multirow{11}{*}{-}\\
  	...
  	&		&				& 5 = 6$^{\text{th}}$ order central + 5$^{\text{th}}$ order upwind	&\\
...
%  & & & &\\
  \hypertarget{iradiation}{iradiation}	& 0	& $x \in \{0, 1, 2, 3, 4, 5, 10\}$ &
  ...
  5 = Radiation using the RTE-RRTMGP library\\
  10 = User defined radiation (use \texttt{rad\_user.f90})\\
  \end{tabular} & - \\
  \qquad	&		&				&						& \\
...
%  sfc\_albedo	& 	& 	& Obsolete & \\
  ...
  uvdamprate	& 0		& $x \in \mathbb{R}, \quad 0 \leq x $	& damp $u$ and $v$ to geostrophic wind with this rate constant & - \\ 
  lfast\_thermo & .false.       & $x\in\{\text{.false.},\text{.true.}\}$ & Enable a faster thermodynamics routine & \\
  lconstexner   & .false.       & $x\in\{\text{.false.},\text{.true.}\}$ & Keep the exner function constant in time, calculated from the initial profile & \\
\end{supertabular}
\end{center}
...
\begin{supertabular}{|l|p{1.2cm}|p{4.7cm}|p{6cm}|l|}
  ...
  \multirow[t]{3}{*}{trestart}	& \multirow[t]{3}{*}{3600}		& \multirow[t]{3}{*}{$x \in \mathbb{R}$} 		& $\text{trestart}>0$:  Each $trestart$ seconds, a restart file is written to disk	& \multirow[t]{3}{*}{s}\\
  ...
  loutdirs	& .false.	& $x\in\{\text{.false.},\text{.true.}\}$		& If .true., per-task output files are stored in subdirectory myidy/ & -\\
\end{supertabular}
...
\begin{supertabular}{|l|p{1.6cm}|p{4cm}|p{6cm}|l|}
  ...
  lanisotrop   & .false.& $x\in\{\text{.false.},\text{.true.}\}$	& Switch for anisotropic subgrid scheme & -\\
...
\begin{supertabular}{|l|p{1.6cm}|p{4cm}|p{6cm}|l|}
  ...
  lxy           & .true.        & $x\in\{\text{.false.},\text{.true.}\}$	& Enable horizontal xy crosssection                     & -\\
  lxz           & .true.        & $x\in\{\text{.false.},\text{.true.}\}$	& Enable vertical xz crosssection	                & -\\
  lyz           & .true.        & $x\in\{\text{.false.},\text{.true.}\}$	& Enable vertical yz crosssection         	        & -\\
  dtav		& dtav\_glob	& $x = n \cdot \text{dtmax}, \quad n \in \mathbb{N}^*$	& Time interval for sampling of statistics	& s\\
  crossheight(1:100)& 1: $x=2$ & $x \in \mathbb{N}, \quad 1 \leq x \leq \text{kmax}$	& Heights of the horizontal crosssection	& -\\
  crossplane	& 2		& $x \in \mathbb{N}, \quad 1 \leq x \leq {\text{jtot}}/{N_\text{processors}}$	& List of locations of vertical (xz) planes& -\\
  crossortho	& 2	& $x \in \mathbb{N}, \quad 1 \leq x \leq \text{itot}$	& List of locations of vertical (yz) planes & - \\
\end{supertabular}
\end{center}
...
\begin{supertabular}{|l|p{1.6cm}|p{4.8cm}|p{6cm}|l|}
  ...
  tmin		& 0.0		& $x \in \mathbb{R}, \quad x \geq 0$	& Start time of field dump	& s \\
  tmax		& $10^8$	& $x \in \mathbb{R}, \quad x \geq 0$	& End time of field dump	& s \\
  lu	       & .true.	        & $x\in\{\text{.false.},\text{.true.}\}$	& Save $u$ 	& -\\
  lv	       & .true.	        & $x\in\{\text{.false.},\text{.true.}\}$	& Save $v$ 	& -\\
  lw	       & .true.	        & $x\in\{\text{.false.},\text{.true.}\}$	& Save $w$ 	& -\\
  lqt	       & .true.	        & $x\in\{\text{.false.},\text{.true.}\}$	& Save $q_t$ 	& -\\
  lql	       & .true.	        & $x\in\{\text{.false.},\text{.true.}\}$	& Save $q_l$ 	& -\\
  lthl	       & .true.	        & $x\in\{\text{.false.},\text{.true.}\}$	& Save $\theta_l$ & -\\
  lbuoy	       & .true.	        & $x\in\{\text{.false.},\text{.true.}\}$	& Save buoyancy field & -\\
  lsv	       & .true.         & $x\in\{\text{.false.},\text{.true.}\}$	& List of booleans for saving sv001, sv002, ... & -\\
  lcli         & .false.        & $x\in\{\text{.false.},\text{.true.}\}$	& save the cli field & - \\
  lclw         & .false.        & $x\in\{\text{.false.},\text{.true.}\}$	& save the clw field & - \\
  lta          & .false.        & $x\in\{\text{.false.},\text{.true.}\}$	& save the ta field  & - \\
  lplw         & .false.        & $x\in\{\text{.false.},\text{.true.}\}$	& save the plw field & - \\
  lpli         & .false.        & $x\in\{\text{.false.},\text{.true.}\}$	& save the pli field & - \\
  lhus         & .false.        & $x\in\{\text{.false.},\text{.true.}\}$	& save the hus field & - \\
  lhur         & .false.        & $x\in\{\text{.false.},\text{.true.}\}$	& save the hur field & - \\
  ltntr        & .false.        & $x\in\{\text{.false.},\text{.true.}\}$	& save the tntr field  & - \\
  ltntrs       & .false.        & $x\in\{\text{.false.},\text{.true.}\}$	& save the tntrs field & - \\
  ltntrl       & .false.        & $x\in\{\text{.false.},\text{.true.}\}$	& save the tntrl field & - \\
\end{supertabular}
\end{center}
...
1: optical depth due to water clouds computed using water droplet effective radius input}& - \\
 ...
 co2\_fraction	& -1		& $x \in \mathbb{R},\quad x \geq 0$	        & If given, CO2 volume fraction is set to this value for RRTMG	& - \\
 ch4\_fraction	& -1		& $x \in \mathbb{R},\quad x \geq 0$	        & If given, CH4 volume fraction is set to this value for RRTMG	& - \\
 n2o\_fraction	& -1		& $x \in \mathbb{R},\quad x \geq 0$	        & If given, N2O volume fraction is set to this value for RRTMG	& - \\
 &&&&\\
...
\newpage
\subsection{Namelist NAMRTERRTMGP}\label{par:rterrtmgp}
This namelist is only used if the RTE-RRTMGP model for GCMs is applied, i.e.  \hyperlink{iradiation}{iradiation}~=~5.
\begin{center}
	  \tablefirsthead{
        \multicolumn{1}{c}{Option} & \multicolumn{1}{c}{Default} & \multicolumn{1}{c}{Possible values} & \multicolumn{1}{c}{Description} & \multicolumn{1}{c}{Unit}\\
        \hline &&&&\\
...
\begin{supertabular}{|l|p{2cm}|p{4cm}|p{6cm}|l|}
  nbatch & jmax &  $x \in \mathbb{N}^*, \quad 1\le x \le \text{jmax}$ & Number of batch of vertical columns sent to the RTE-RRTMGP routines, chosen such that jmax is a multiple of nbatch & -\\
  doclearsky	& .false.	& $x\in\{\text{.false.},\text{.true.}\}$	& Perform additional clear-sky calculations	& -\\
  usepade	& .false.	& $x\in\{\text{.false.},\text{.true.}\}$	& Use Pade coefficients to compute cloud optical properties instead of look up tables	& -\\
\end{supertabular}
\end{center}
...
\newpage
\subsection{Namelist NAMMSEBUDG}\label{par:msebudget}
...
\begin{supertabular}{|l|p{2cm}|p{4cm}|p{6cm}|l|}
  lmsebudg	& .false.	& $x\in\{\text{.false.},\text{.true.}\}$	        & Switch to turn on moist static energy budget output & -\\
  dtav		& dtav\_glob	& $x = n \cdot \text{dtmax}, \quad n \in \mathbb{N}^*$	& Time interval for sampling of variance budget	& s \\
\end{supertabular}
\end{center}
...
\begin{supertabular}{|l|p{2cm}|p{4cm}|p{6cm}|l|}
  ...
  lsamptendu & .false.        & $x\in\{\text{.false.},\text{.true.}\}$        & Switch for (conditional) sampling of u tendencies & -\\
  lsamptendv & .false.        & $x\in\{\text{.false.},\text{.true.}\}$        & Switch for (conditional) sampling of v tendencies & -\\
  lsamptendw & .false.        & $x\in\{\text{.false.},\text{.true.}\}$        & Switch for (conditional) sampling of w tendencies & -\\
  lsamptendthl & .false.        & $x\in\{\text{.false.},\text{.true.}\}$        & Switch for (conditional) sampling of thl tendencies & -\\
  lsamptendqt & .false.        & $x\in\{\text{.false.},\text{.true.}\}$        & Switch for (conditional) sampling of qt tendencies & -\\
  lsamptendqr & .false.        & $x\in\{\text{.false.},\text{.true.}\}$        & Switch for (conditional) sampling of qr tendencies & -\\
  lsamptendnr & .false.        & $x\in\{\text{.false.},\text{.true.}\}$        & Switch for (conditional) sampling of nr tendencies & -\\
  lprocblock & .false.        & $x\in\{\text{.false.},\text{.true.}\}$        & switch to write tendencies averaged over processor blocks instead of a horizontal slab & -\\
  ltendleib & .false.        & $x\in\{\text{.false.},\text{.true.}\}$        & switch to turn on leibniz tendencies & -\\
  ltenddec & .false.        & $x\in\{\text{.false.},\text{.true.}\}$        & switch to get variables needed to scale-decompose (processor-averaged) advective tendencies & -\\
\end{supertabular}
\end{center}
...
\begin{supertabular}{|l|p{1.6cm}|p{3.5cm}|p{6cm}|l|}
  ...
  min\_horv     & 0.1   & $x \in \mathbb{R}, \quad x \ge 0$ & Minimum surface wind velocity for drag calculation & m s$^{-1}$\\
...
%  planttype & 3	& $x \in \{3, 4, 5\}$	& Switch between (C)3 and (C)4 plants for A-g$_{\rm s}$ & - \\
