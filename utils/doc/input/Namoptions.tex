...
\begin{supertabular}{|l|p{1.6cm}|p{4cm}|p{6cm}|l|}
  ...
  lnoclouds	& .false.	& $x\in\{\text{.false.},\text{.true.}\}$		& Switch to disable $\theta_l$ calculations	& -\\
  \qquad	&		&				&							&\\
...
1: optical depth due to water clouds computed using water droplet effective radius input}& - \\
 ...
 doperpetual	& .false.	& $x\in\{\text{.false.},\text{.true.}\}$	& Switch to not use a diurnal cycle	& -\\
 doseasons	& .false.	& $x\in\{\text{.false.},\text{.true.}\}$	& Switch to account for changes in Earth's position from day to day	& -\\
